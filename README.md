...
| 📊 组合分析 | 总市值/成本/盈亏（含实物黄金）、按市场/标的分配、**净值曲线（v34：新增第三条「累计收益（含已实现）」折线，采用双 Y 轴——左轴 总市值/总成本、右轴 累计收益，黄色 `#d29922`，历史无数据段自动断点、绝不虚构）**、最大回撤、累计收益；**收益分析模块**（基于每日净值快照：日/月/年/累计收益 + 每持仓区间明细 + 柱状图/日历图，**日历图每个收益格子上显示当天收益数字**，v33，金额取整、≥1万缩略万、悬停见精确值；**v34：日历每行改为「周一 → 周日」排列，且每个格子左上角以数字标注当天日期；收益明细与组合总收益的可编辑输入框改为「正数显示 + 号并为红字 / 负数显示 - 号并为绿字」（修复此前正负配色被 CSS 覆盖而不生效的问题）**）；**总览新增「当日收益」卡片**（v32，位于总成本与当前持仓收益之间：实时行情口径当日收益总和，正红负绿，点击弹出各产品当日收益明细与缺失提示，接口 `/api/portfolio/day-pnl`） |
| 📄 报表 | **持仓汇总**（含实物黄金；**表头支持按市值 / 成本 / 盈亏 / 收益率点击升降序排序，默认市值降序**，v33）+ **交易流水**，均一键导出 CSV / 打印 |
| 📰 财经资讯 | 新浪滚动财经新闻 |
...
- **组合分析**：查看总市值/成本/盈亏（含实物黄金）、按市场或标的的持仓分配、净值曲线与最大回撤。**v34：净值曲线（每日快照）图表新增第三条「累计收益（含已实现）」折线**——因累计收益与总市值量级差约 5 倍（实测约 8 万 vs 41 万），图表采用**双 Y 轴**（左轴 总市值/总成本，右轴 累计收益），累计收益线为**黄色 `#d29922`**、右轴刻度与轴线同色便于识别；数据取自每日快照的累计收益列，**历史无数据的那几天保持断点、不连线也不填充虚拟值**；悬停 tooltip 显示日期与三条线的金额（千分位 + ¥ 前缀，缺失显示「—」）。**新增：饼图点击扇区弹窗**——点击「资产分配」饼图的任一扇区（市场/标的），弹出该类型下所有产品明细（产品代码、产品名称、市值、占该类型百分比），便于查看某类型下的具体持仓分布。
- **报表**：**持仓汇总表默认按市值降序**，点击「市值 / 成本 / 盈亏 / 收益率」表头切换该列升降序（同列再点翻转，箭头指示当前方向，合计行不参与排序恒在末行）；下方为全部交易流水。持仓（含实物黄金）与交易明细均可一键导出 CSV、打印。
