...
| 💼 持仓管理 | 买卖交易记录录入，**实物黄金克数与成本价编辑**（人民币/克，模块置于页面最下方），加权平均成本法自动聚合持仓，实时市值与盈亏；**持仓卡片顶部两栏并列展示「当前持仓收益 | 累计收益」并文字区分**（v32，正红负绿）；**默认按持仓收益排序**（可切换按市值 / 成本）、**置顶产品**（数量不限，后端持久化 + 跨机器迁移）、**每页 20 条翻页**；**实物黄金买卖流水每页 5 条** + 翻页；**数据导入导出卡片内 v33 起内置危险区「🗑 一键清空全部持仓与交易记录」**（两层确认：第一层列清空范围 + 可选是否连自选/指数一起清，第二层须输入「确认清空」；清空后自动重写根目录 `持仓数据_export.json`，接口 `POST /api/data/clear-all`） |
| 📊 组合分析 | 总市值/成本/盈亏（含实物黄金）、按市场/标的分配、净值曲线、最大回撤、累计收益；**收益分析模块**（基于每日净值快照：日/月/年/累计收益 + 每持仓区间明细 + 柱状图/日历图，**日历图每个收益格子上显示当天收益数字**，v33，金额取整、≥1万缩略万、悬停见精确值）；**总览新增「当日收益」卡片**（v32，位于总成本与当前持仓收益之间：实时行情口径当日收益总和，正红负绿，点击弹出各产品当日收益明细与缺失提示，接口 `/api/portfolio/day-pnl`） |
